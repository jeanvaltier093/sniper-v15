            ...
            try:
                ...
                if score >= score_min and not adx_block and not h1_block and comment == "-":
                    if trend_up and breakout_up:
                        signal = "ACHAT 🚀"
                        # SL sous le plus bas 20j ou ATR (le plus protecteur)
                        sl = min(lowest_20d, close - atr*2)
                        # TP avant la prochaine résistance majeure ou 2.1 RR
                        potential_tp = close + (close-sl)*2.1
                        tp = max(highest_20d, potential_tp)
                    elif not trend_up and breakout_dn:
                        signal = "VENTE 🔻"
                        # SL au dessus du plus haut 20j ou ATR
                        sl = max(highest_20d, close + atr*2)
                        # TP avant support majeur
                ...
                if sl and tp:
                    ...
                    if rr < 1.2: # On refuse le trade si le mur statique est trop proche
                        signal = "ATTENDRE"
